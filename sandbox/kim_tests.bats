...
@test "good path test appsody run nodejs" {
    ...
    cancel $PID
    #[ "$status" -eq 0 ]
    #run kill -2 -$(ps xao pgid,command | grep '[a]ppsody run' | awk '{print $1}')
    #echo "${output}"
    #echo "${output}" > $LOGDIR/good-path-appsody-run-nodejs-kill.log

    # sleep to let the stop process complete
    #appsody stop --name nodejs-dev
    sleep 11
    ...
    #run appsody ps | awk 'NR==2{print $1}'
    #echo "${output}"
    #echo "${output}" > $LOGDIR/good-path-appsody-run-nodejs-ps-after-kill.log
    #[ "$status" -eq 0 ]  
